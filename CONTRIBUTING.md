...
- All downloaded binaries are verified against upstream SHA256 checksums
- `apt-get install` always includes `--no-install-recommends` and cleanup in the same layer (`rm -rf /var/lib/apt/lists/*`)
- Multi-stage builds minimize final image size
- The container runs as a non-root user at runtime
